...
export const createSubCategory = expressAsyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (categoryExist.subCategoriesCount === limitedForSubCategory()) {
      return next(
        new ApiError(
          {
            en: `you can't add more than ${limitedForSubCategory()} subcategories in each category`,
            ar: `لا يمكنك إضافة أكثر من ${limitedForSubCategory()} تصنيفات فرعية في كل تصنيف`,
          },
          StatusCodes.NOT_FOUND
        )
      );
