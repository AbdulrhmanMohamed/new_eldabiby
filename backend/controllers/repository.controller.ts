...
export const createRepository = expressAsyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    for (const product of products) {
      ...
      const eachProduct: IProduct | null = await Product.findById(
        productId
      ).exec();
...
export const getAllRepositoriesForAllProducts = expressAsyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    let onlineItems = order?.onlineItems?.items;
    let cashItems = order?.cashItems?.items;
    let productsWithReops: any = [];
