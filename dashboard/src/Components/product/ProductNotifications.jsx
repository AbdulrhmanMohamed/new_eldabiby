...
const ProductNotifications = ({
  ...
  const { colors, customColors, btnStyle } = useTheme();
  const [open, setOpen] = useState(false);
  const [getAllUsers] = useLazyGetAllUsersQuery();
  const [changedUsers, setChangedUsers] = useState([]);
  const [selectedUsers, setSelectedUsers] = useState([]);
  // const roles = ["rootAdmin", "subAdmin", "adminA", "adminB", "adminC", "user"];
  // const arabicRoles = {
  //   rootAdmin: "مشرف",
  //   subAdmin: "مشرف فرعي",
  //   adminA: "مشرف أ",
  ...
  // };
  useEffect(() => {
    getAllUsers(`role=${productNotificationValues.role}`)
      .unwrap()
      .then((res) => {
        setChangedUsers(res.data);
        setSelectedUsers([]);
      })
      .catch(() => {
        setChangedUsers([]);
      });
  }, [productNotificationValues.role]);
  useEffect(() => {
    if (selectedUsers.includes("all") || selectedUsers.includes("الجميع")) {
      setProductFieldValue("receiver", "all");
      setSelectedUsers(
        selectedUsers.filter((item) => item !== "all" || item !== "الجميع")
      );
    } else {
      let filteredSelected = changedUsers.filter((user) =>
        selectedUsers.includes(
          user[user.name ? "name" : user.email ? "email" : "phone"]
        )
      );
      let users_Ids = filteredSelected.map((el) => el._id).join(",");
      setProductFieldValue("receiver", users_Ids);
    }
  }, [selectedUsers]);
