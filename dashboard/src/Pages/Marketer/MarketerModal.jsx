...
function MarketerModal ({ open, setOpen, dataMarketer, setDataMarketer }) {
  ...
  const generateUrl = () => {
    ...
    if (values.discountDepartment.key === 'allProducts') {
      ...
      const sub = options.subcategories.find(
        subcategory => subcategory?._id === values?.discountDepartment?.value[0]
      )
  ...
  const {
    ...
    validationSchema: Yup.object({
      ...
      commissionMarketer: Yup.string().required(language === 'en' ? 'Required' : 'مطلوب')
      .test(
        'sum-of-discount-commission', 
        (language === "en" ? 'Sum of discount and commissionMarketer should be less than 100' : 'مجموع الخصم والعمولة يجب ان يكون اقل من 100'),
        function (discount) {
      ...
      <form onSubmit={handleSubmit}>
        <FormControl sx={{ width: '100%', gap: 3 }}>
          ...
          >
            <Stack sx={{ width: '100%' }}>
              <Typography sx={{ color: customColors.label, mb: '4px' }}>
                {language === 'en' ? 'Value of Discount (in percentage)' : '(بالنسبة المئوية) قيمة الخصم'}
              </Typography>
              <TextField
                name='discount'
                type='number'
            ...
            <Stack sx={{ width: '100%' }}>
              <Typography sx={{ color: customColors.label, mb: '4px' }}>
                {language === 'en' ? 'commission (in percentage)' : '(بالنسبة المئوية) العمولة'}
              </Typography>
              <TextField
