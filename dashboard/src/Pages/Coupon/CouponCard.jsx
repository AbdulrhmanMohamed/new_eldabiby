...
function CouponCard({ coupon, handleOpenEdit }) {
  ...
  const handleDeleteCoupon = () => {
    deleteCoupon(coupon._id)
      ...
      .catch((error) => {
        toast.error(language === "en" ? error?.data?.error_en : error?.data?.error_ar);
      });
  };
  return (
    <>
      <Paper
        elevation={1}
        sx={{
          bgcolor: coupon.active ? customColors.card : customColors.cardNotActive,
          p: 2,
          borderRadius: 5,
          gap: 5,
          display: "flex",
      ...
      >
        <Stack sx={{ width: "100%", gap: 1 }}>
          ...
          <Stack sx={{ flexDirection: "row", alignItems: "center", gap: "5px" }}>
            ...
            <Typography sx={{ fontWeight: "bold", fontSize: "14px" }}>
              {language === "en" ? coupon.discountDepartment.key : arabicKeys[coupon.discountDepartment.key]}
            </Typography>
          </Stack>
